 ...
 */
@Api(value = "/WalletPay", description = "钱包充值 接口; Responseble:cww")
@RestController
...
WalletPayController {
    ...
    @Autowired
    MszWalletBillService billService;

    @ApiImplicitParams({
            @ApiImplicitParam(name = "openId", value = "用户OpendId", required = true, paramType = "query"),
            @ApiImplicitParam(name = "walletNo", value = "钱包流水号", required = true, paramType = "query"),
            @ApiImplicitParam(name = "total", value = "充值金额", required = true, paramType = "query")
    })
    @ApiOperation(value = "请求支付接口")
    @PostMapping("/wxPay")
    public RespEntity wxPay(String openId, String walletNo, Integer total) {
        total = 1;
        try {
            //生成的随机字符串
            ...
            packageParams.put("out_trade_no", walletNo + "");//商户订单号,自己的订单ID
            ...
            packageParams.put("notify_url", WechatConfig.wallet_notify_url);//支付成功后的回调地址
            ...
            String xml = "<xml>" + "<appid>" + WechatConfig.appid + "</appid>"
                    ...
                    + "<notify_url>" + WechatConfig.wallet_notify_url + "</notify_url>"
                    + "<openid>" + openId + "</openid>"
                    + "<out_trade_no>" + walletNo + "</out_trade_no>"
